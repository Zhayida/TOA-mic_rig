...
%[I,J,D]=find(dRound);
%[xopt,yopt]=bundletoa(D,I,J,R,S); 
%[xopt,yopt]=bundleToaMicRigsUncalibMics3DSounds2D(D,I,J,R,S(1:2,:),0.01,100);

%dTest = toa_calc_d_from_xy(xopt,[yopt ] );
%dTest = toa_calc_d_from_xy(xopt,[yopt ; zeros(1,size(yopt,2))] );

%[norm(dTest-dRound) norm(d-dRound)]
...
eqs(4,1)=vv1*vu(1)-vu(8)^2;
eqs(5,1)=vu(2)*vu(3)-vu(9)^2;
eqs(6,1)=vu(4)*vu(5)-vu(10)^2;
eqs(7,1)=vu(6)*vu(7)-u78^2;
